...
def search_leagues(api_key, name):
    """GET /leagues/search/{name} with seasons and country attached, so
    we can offer a season picker and build a "Country League Name" CSV
    filename. NOTE: the base league object only carries `country_id`
    (confirmed against SportMonks' docs) - `include=country` is what
    attaches the actual country name."""
    url = f"{BASE_URL}/leagues/search/{name}"
    resp = requests.get(
        url, params={"api_token": api_key, "include": "seasons;country"}, timeout=30
    )
...
league_name_for_file = country_name_for_file = None
if st.session_state.league_matches:
    ...
    league_name_for_file = chosen_league.get("name")
    country_name_for_file = (chosen_league.get("country") or {}).get("name")
    ...
    if not seasons:
        ...
        st.caption(
            "Requests stats for EVERY fixture, including ones that haven't "
            "been played yet - those will simply come back with blank "
            "goals/stat columns until SportMonks has data for them."
        )
        if st.button("Get season fixtures & stats"):
            try:
                ...
                st.write(f"{total} fixture(s) in this season - requesting stats for all of them.")
                ...
                played = sum(1 for r in rows if r["home_goals"] is not None)
                st.success(
                    f"Done - pulled {len(rows)} fixture(s) total "
                    f"({played} played, {len(rows) - played} not yet played)."
                )
            except requests.HTTPError as exc:
                st.error(f"SportMonks returned an error: {exc}")
            except Exception as exc:  # noqa: BLE001
...
if not st.session_state.season_rows:
    ...
    raw_name = "_".join(
        part for part in [country_name_for_file, league_name_for_file, season_name] if part
    ) or "sportmonks_season"
    safe_name = "".join(c if c.isalnum() else "_" for c in raw_name).strip("_") + ".csv"
